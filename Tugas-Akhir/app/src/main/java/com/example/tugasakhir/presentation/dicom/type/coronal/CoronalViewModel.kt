...
class CoronalViewModel(
    ...
    fun saveAnnotationForSliceAndUpload(context: Context, dicomId: String, sliceIndex: Int, onComplete: (Boolean) -> Unit) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.e("CoronalViewModel", "Gagal upload anotasi: ${e.message}")
                onComplete(false)
            }
        }
    }
    ...
    fun loadFile(
        ...
        viewModelScope.launch {
            try {
                ...
                val result = coronalRepository.readBinFile(file.absolutePath)
    ...
    fun loadAnnotationsFromFirestore(
        ...
        FirebaseFirestore.getInstance()
                ...
                for (doc in query.documents) {
                    ...
                    viewModelScope.launch(Dispatchers.IO) {
                        try {
                            ...
                            Log.e("CoronalViewModel", "Gagal memuat anotasi slice $index: ${e.message}")
                        }
                    }
                }
            ...
            .addOnFailureListener {
                Log.e("CoronalViewModel", "Gagal mengambil anotasi ($orientation): ${it.message}")
            }
    }
    ...
    suspend fun downloadFileFromFirebaseUrl(context: Context, url: String): File = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            Log.e("CoronalViewModel", "Gagal unduh file: ${e.message}")
            throw IOException("Gagal download dari URL")
        }
    }

    fun updateBitmapForY(y: Int, binaryData: ByteArray, dicomId: String? = null) {
        val result = coronalRepository.processBitmap(y, binaryData)
