...
class SagitalViewModel(
    ...
    fun loadFile(
        ...
        viewModelScope.launch {
            try {
                ...
                val result = sagitalRepository.readBinFile(file.absolutePath)
    ...
    fun loadAnnotationsFromFirestore(
        ...
        FirebaseFirestore.getInstance()
                ...
                for (doc in query.documents) {
                    ...
                    viewModelScope.launch(Dispatchers.IO) {
                        try {
                            ...
                            Log.e("SagitalViewModel", "Gagal memuat anotasi slice $index: ${e.message}")
                        }
                    }
                }
            ...
            .addOnFailureListener {
                Log.e("SagitalViewModel", "Gagal mengambil anotasi ($orientation): ${it.message}")
            }
    }
    ...
    private fun downloadAnnotationBitmap(url: String, width: Int, height: Int): Bitmap {
        ...
        if (bytes.size < expectedSize) {
            Log.w("SagitalViewModel", "Ukuran data anotasi kurang: hanya ${bytes.size} byte, seharusnya $expectedSize")
        }
    ...
    suspend fun downloadFileFromFirebaseUrl(context: Context, url: String): File = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            Log.e("SagitalViewModel", "Gagal unduh file: ${e.message}")
            throw IOException("Gagal download dari URL")
        }
    }

    fun updateBitmapForX(x: Int, binaryData: ByteArray, dicomId: String? = null) {
        val result = sagitalRepository.processBitmap(x, binaryData)
        if (result is ResultState.Success) {
            val bitmap = result.data
            imageWidth = bitmap?.width ?: 256
            imageHeight = bitmap?.height ?: 256
